...
def init_models(device):
    vit_model = vit_b_16()
    checkpoint = torch.load("/scratch/yl10337/FontDiffuser/vit_b_16-c867db91.pth", map_location="cpu")
...
def compute_fid_from_files(files1, files2, device):
    fid = FrechetInceptionDistance(
        feature=2048,
        feature_extractor_weights_path="/scratch/yl10337/FontDiffuser/pt_inception-2015-12-05-6726825d.pth"
    ).to(device)
...
if __name__ == "__main__":
    ...
    if args.folder:
        results = evaluate_folder(args.input1, args.input2, args)
    else:
