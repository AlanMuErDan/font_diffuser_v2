...
def sampling(args, pipe, content_image=None, style_image=None, output_file="out_single.png"):
    if not args.demo:
    ...
    with torch.no_grad():
        ...
        if args.save_image:
            ...
            save_single_image(save_dir=args.save_image_dir, image=images[0], filename=output_file)
            # if args.character_input:
...
if __name__=="__main__":
    ...
    if args.content_dir and args.style_dir:
            ...
            common_files = list(set(content_images) & set(style_images))
            common_files.sort()  # sort numerically if needed

            for fname in common_files:
                content_path = os.path.join(args.content_dir, fname)
                style_path = os.path.join(args.style_dir, fname)

                args.content_image_path = content_path
                args.style_image_path = style_path

                print(f"Processing {fname} ...")
                out_image = sampling(args=args, pipe=pipe, output_file=fname)
    else:
